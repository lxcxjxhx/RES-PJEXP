
<!-- ═══════════════════════════════════════════════════════════════════════════
     安全风信子 | 信息安全×AI 双域实践者 — Personal Achievement Portfolio
     ═══════════════════════════════════════════════════════════════════════════ -->

<div align="center">

**语言 / Language:** [中文](README.md) | [English](README.en.md)

</div>

---

<div align="center">

<!-- Banner -->
<table align="center" width="90%" style="margin: 0 auto; border-collapse: collapse;">
<tr><td align="center" style="background: #1a1a2e; padding: 40px 20px; border-radius: 12px;">

<img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=20&duration=3500&pause=500&color=00D4FF&center=true&vCenter=true&width=600&lines=%E5%AE%89%E5%85%A8%E9%A3%8E%E4%BF%A1%E5%AD%90+%7C+%E4%BF%A1%E6%81%AF%E5%AE%89%E5%85%A8%C3%97AI+%E5%AE%9E%E8%B7%B5%E8%80%85;+%E7%94%A8%E4%BB%A3%E7%A0%81%E9%94%BB%E9%80%A0%E6%99%BA%E8%83%BD%E6%97%B6%E4%BB%A3%E7%9A%84%E5%9D%9A%E7%9B%BE%E4%B8%8E%E5%88%A9%E7%9B%BE" alt="Typing SVG" />

<br/><br/>

<img src="https://img.shields.io/badge/阿里云-专家博主-FF6A00?style=for-the-badge&logo=alibabacloud&logoColor=white" alt="阿里云专家博主" />
<img src="https://img.shields.io/badge/华为云-专家博主-C7000B?style=for-the-badge&logo=huawei&logoColor=white" alt="华为云专家博主" />
<img src="https://img.shields.io/badge/腾讯云-创作之星-0052D9?style=for-the-badge&logo=tencentqq&logoColor=white" alt="腾讯云创作之星" />
<img src="https://img.shields.io/badge/中美创客-特等奖-FFD700?style=for-the-badge&logo=award&logoColor=white" alt="中美创客特等奖" />

<br/><br/>

<img src="https://img.shields.io/badge/August_2026-1a1a2e?style=flat-square&logo=github&logoColor=white" alt="August 2026" />

</td></tr>
</table>

</div>

---

<div align="center">

## About Me

</div>

<br/>

<div align="center">
<table align="center" width="85%" style="margin: 0 auto;">
<tr><td style="padding: 24px; line-height: 1.8; font-size: 15px;">

**钱佳宏 (Qian Jiahong) · 上海**

三年前，我在 CSDN 写下第一篇安全笔记，记录一次应急响应的完整排查过程——从日志分析到最终定位攻击者 IP。那是"安全风信子"这个名字的起点。

后来，我把写博客的习惯变成了写开源工具的习惯。从第一个 PyPI 包发布时的忐忑，到在 HuggingFace 上微调出能理解攻击日志的模型，再到用 Multi-Agent 架构构建 HOS-LS——一个让 AI 自动分析代码漏洞、推理攻击链的系统。这条路从"记录"走到"创造"，从"个人笔记"走到 **168** 个被社区采纳的 Pull Request。

现在，我在 **信息安全 × 大模型工程** 的交叉地带全速前进：用 vLLM 部署生产级推理服务，用 MCP 协议构建可控的 Agent 系统，用 Prompt Engineering 把 LLM 变成安全分析师的副驾驶。

**1,467 篇原创文章** 不是数字——是 1,467 次"搞明白一件事"的承诺。每一个项目、每一篇专栏，都源于一个真实的困惑和一次诚实的探索。

上海建桥学院在读 · ✉️ [aqfxz_zh@qq.com](mailto:aqfxz_zh@qq.com)

</td></tr>
</table>
</div>

---

<div align="center">

## What I Built

</div>

<br/>

<!-- HOS-LS -->
<div align="center">
<table align="center" width="85%" style="margin: 0 auto;">
<tr><td align="center" style="padding: 20px;">

### [HOS-LS](https://github.com/lxcxjxhx/HOS-LS) — AI 驱动的代码安全分析与攻击链推理系统

> 传统静态扫描只能告诉你"这里有 SQL 注入"，却无法回答"攻击者会怎么利用它"。HOS-LS 要做的，是让 AI 像安全专家一样，读懂代码、理解漏洞、推理出完整的攻击路径。

<p>
  <img src="https://img.shields.io/badge/AI_语义分析-深层代码理解-2c3e50?style=flat-square" />
  <img src="https://img.shields.io/badge/攻击链推理-端到端路径-2c3e50?style=flat-square" />
  <img src="https://img.shields.io/badge/Multi--Agent-多智能体协同-2c3e50?style=flat-square" />
  <img src="https://img.shields.io/badge/Exploit_生成-自动化验证-2c3e50?style=flat-square" />
</p>

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
  <img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white" />
  <img src="https://img.shields.io/badge/LangChain-7C3AED?style=flat-square" />
</p>

</td></tr>
</table>
</div>

<!-- HOS_SKILL_WORKFLOW -->
<div align="center">
<table align="center" width="85%" style="margin: 0 auto;">
<tr><td align="center" style="padding: 20px;">

### [HOS_SKILL_WORKFLOW](https://github.com/lxcxjxhx/HOS_SKILL_WORKFLOW) — HOS 工作流提示词工厂

> 写 Prompt 不难，难的是让 Prompt 在生产环境中稳定、可复现、可编排。这个项目把零散的 Prompt 系统化为可组合的工作流，像搭积木一样组装 AI 能力。

<p>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white" />
  <img src="https://img.shields.io/badge/Prompt_Engineering-2c3e50?style=flat-square" />
  <img src="https://img.shields.io/badge/Workflow-2c3e50?style=flat-square" />
  <img src="https://img.shields.io/badge/⭐_2-f59e0b?style=flat-square&logo=github&logoColor=white" />
</p>

</td></tr>
</table>
</div>

<!-- Other Repos -->
<div align="center">
<table align="center" width="85%" style="margin: 0 auto;">
<tr>
<td width="33%" align="center" valign="top" style="padding: 16px;">

**[HOS-MATCH-PROJECT](https://github.com/lxcxjxhx/HOS-MATCH-PROJECT)**
<sub>Intel AI 竞赛 · 知识图谱 + 大模型威胁检测</sub>
<br/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/Qwen2.5-2c3e50?style=flat-square" />
<img src="https://img.shields.io/badge/React-22D3EE?style=flat-square&logo=react&logoColor=black" />

</td>
<td width="33%" align="center" valign="top" style="padding: 16px;">

**[BOS-FS](https://github.com/lxcxjxhx/BOS-FS)**
<sub>安全工具集 · Python 实用脚本</sub>
<br/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />

</td>
<td width="33%" align="center" valign="top" style="padding: 16px;">

**[BOS-NI](https://github.com/lxcxjxhx/BOS-NI)**
<sub>安全知识库 · 知识沉淀与索引</sub>
<br/>
<img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white" />

</td>
</tr>
</table>
</div>

<p align="center">
  <a href="https://github.com/lxcxjxhx"><img src="https://img.shields.io/badge/查看完整_GitHub_主页-2c3e50?style=for-the-badge&logo=github&logoColor=white" alt="View GitHub"/></a>
</p>

---

<div align="center">

## What I Write About

</div>

<br/>

<div align="center">
<table align="center" width="85%" style="margin: 0 auto;">
<tr><td style="padding: 20px; line-height: 1.8;">

### 核心专栏

**[HOS-AI | 大模型系统工程 × AI变现实战](https://security-hyacinth.blog.csdn.net/)**（150 篇）
> 从 Prompt 工程到 Multi-Agent 架构，从 vLLM 部署到 MCP 协议实战。这是我日常最密集的输出——每一篇都对应一个我亲手踩过的坑。如果你想系统学大模型工程，从这里开始最不浪费时间。

**[MCP 工程实战：构建可控的 AI 工具与 Agent 系统](https://security-hyacinth.blog.csdn.net/)**（69 篇）
> MCP 协议正在改变 Agent 与工具的交互方式。这个专栏从协议原理讲到生产级实现，适合想让 LLM 真正"动手做事"的开发者。

**[vLLM 生产级 LLM 推理引擎实战](https://security-hyacinth.blog.csdn.net/)**（70 篇）
> 本地部署大模型不只是跑通就好——吞吐量、延迟、显存优化才是真战场。这个专栏记录了我在 vLLM 生产化路上的全部实战经验。

</td></tr>
</table>
</div>

<br/>

<details>
<summary><b>全部专栏一览（点击展开）</b></summary>

<br/>

<table align="center" width="85%" style="margin: 0 auto;">
<thead>
<tr>
<th align="left">专栏</th>
<th align="center">篇数</th>
<th align="left">标签</th>
</tr>
</thead>
<tbody>
<tr><td><strong>HOS-AI | 大模型系统工程 × AI变现实战</strong></td><td align="center">150</td><td>（核心）</td></tr>
<tr><td><strong>MCP 工程实战：构建可控的 AI 工具与 Agent 系统</strong></td><td align="center">69</td><td>（系统化教程）</td></tr>
<tr><td><strong>vLLM 生产级 LLM 推理引擎实战（2026）</strong></td><td align="center">70</td><td>（生产级实战）</td></tr>
<tr><td><strong>日学日新 · HF/MS 模型进化录</strong></td><td align="center">45</td><td>（持续更新）</td></tr>
<tr><td><strong>HOS工程急救库 | Git·Python·AI环境问题速查</strong></td><td align="center">41</td><td>（工具速查）</td></tr>
<tr><td><strong>专业帮助文档特辑：即用模板与专家解决方案</strong></td><td align="center">40</td><td>（实用模板）</td></tr>
<tr><td><strong>如果夜神月学会 AIx信安：77个技术脑洞</strong></td><td align="center">76</td><td>（创意系列）</td></tr>
<tr><td><strong>L的AIx信安推理：76次防御与溯源基拉</strong></td><td align="center">76</td><td>（创意系列）</td></tr>
<tr><td><strong>HOS-CTF | 从入门到国赛的攻防之路 2026</strong></td><td align="center">20</td><td>（付费）</td></tr>
<tr><td><strong>2025 大模型+AIGC</strong></td><td align="center">165</td><td>（归档）</td></tr>
<tr><td><strong>2025 信息安全CTF全题型&题解</strong></td><td align="center">160</td><td>（归档）</td></tr>
<tr><td><strong>个人思考分享（安全风信子精选专栏）</strong></td><td align="center">13</td><td>（精选）</td></tr>
</tbody>
</table>

</details>

<br/>

---

<div align="center">

## Numbers & Milestones

</div>

<br/>

<table align="center" width="80%" style="margin: 0 auto;">
<thead>
<tr>
<th align="left">指标</th>
<th align="center">数字</th>
<th align="left">注释</th>
</tr>
</thead>
<tbody>
<tr>
<td>原创文章</td>
<td align="center"><b>1,467</b></td>
<td>CSDN 全部原创，无转载。每篇对应一个实际项目或真实踩坑经历。</td>
</tr>
<tr>
<td>已合并 PR</td>
<td align="center"><b>168</b></td>
<td>仅统计 merged 状态。涵盖 LlamaFactory、trivy、thc-hydra 等项目。</td>
</tr>
<tr>
<td>PyPI 发布包</td>
<td align="center"><b>7</b></td>
<td>安全工具链相关，可直接 <code>pip install</code> 使用。</td>
</tr>
<tr>
<td>GitHub 年度贡献</td>
<td align="center"><b>1,966</b></td>
<td>相当于每天 5.4 次 commit，连续不断。</td>
</tr>
<tr>
<td>竞赛荣誉</td>
<td align="center"><b>特等奖</b></td>
<td>中美青年创客大赛。同期参加 Intel AI 竞赛等多项国家级赛事。</td>
</tr>
<tr>
<td>平台认证</td>
<td align="center"><b>3</b></td>
<td>阿里云 / 华为云 / 腾讯云 专家博主。CET-6 · 网络安全应急响应（中级）</td>
</tr>
</tbody>
</table>

<br/>

---

<div align="center">

## What I'm Exploring

</div>

<br/>

<table align="center" width="80%" style="margin: 0 auto;">
<thead>
<tr>
<th align="left">领域</th>
<th align="left">实践内容</th>
</tr>
</thead>
<tbody>
<tr>
<td><strong>LLM Applications</strong></td>
<td>Qwen2.5 / LLaMA 微调与本地部署、Multi-Agent 系统、Prompt Engineering、vLLM 生产级部署、MCP 工程</td>
</tr>
<tr>
<td><strong>Cybersecurity</strong></td>
<td>CTF 实战（国赛级）、红队自动化工具、AI 驱动代码安全分析 (HOS-LS)、攻击链推理</td>
</tr>
<tr>
<td><strong>Full-Stack</strong></td>
<td>React + Taro 前端 + FastAPI 后端，中小型 Web 与小程序项目</td>
</tr>
<tr>
<td><strong>Healthcare AI</strong></td>
<td>非临床视角下的对话系统与数据可视化探索</td>
</tr>
<tr>
<td><strong>DevOps</strong></td>
<td>本地 AI 环境搭建、CUDA/WSL2 问题排查、uv 包管理</td>
</tr>
</tbody>
</table>

<br/>

---

<div align="center">

## Tech Stack

</div>

<br/>

<div align="center">

**Languages & Frameworks**

<img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/PyTorch-EE4C2C?style=for-the-badge&logo=pytorch&logoColor=white" />
<img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" />
<img src="https://img.shields.io/badge/React-61DAFB?style=for-the-badge&logo=react&logoColor=black" />
<img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=node.js&logoColor=white" />

<br/><br/>

**AI / LLM**

<img src="https://img.shields.io/badge/vLLM-生产级推理-6366F1?style=for-the-badge&logo=python&logoColor=white" />
<img src="https://img.shields.io/badge/FastAPI-10B981?style=for-the-badge&logo=fastapi&logoColor=white" />
<img src="https://img.shields.io/badge/MCP-工程化-EC4899?style=for-the-badge&logo=typescript&logoColor=white" />
<img src="https://img.shields.io/badge/Taro-跨端开发-000000?style=for-the-badge&logo=taro&logoColor=white" />

<br/><br/>

**DevOps & Tools**

<img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white" />
<img src="https://img.shields.io/badge/GitHub_Actions-2088FF?style=for-the-badge&logo=githubactions&logoColor=white" />
<img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white" />

</div>

---

<div align="center">

## Let's Connect

</div>

<br/>

<div align="center">
<table align="center" width="80%" style="margin: 0 auto;">
<tr><td style="padding: 20px; line-height: 1.8; text-align: center;">

如果你在做安全工具、LLM 应用、或者只是对 AI × Security 感兴趣——很乐意聊聊。

合作、交流、甚至只是想讨论一个技术问题，都可以随时找我。

上海 · ✉️ [aqfxz_zh@qq.com](mailto:aqfxz_zh@qq.com)

<a href="https://github.com/lxcxjxhx"><img src="https://img.shields.io/badge/GitHub-2c3e50?style=for-the-badge&logo=github&logoColor=white" /></a>
<a href="https://security-hyacinth.blog.csdn.net/"><img src="https://img.shields.io/badge/CSDN-2c3e50?style=for-the-badge&logo=c&logoColor=white" /></a>
<a href="https://pypi.org/user/security_hyacinth/"><img src="https://img.shields.io/badge/PyPI-2c3e50?style=for-the-badge&logo=pypi&logoColor=white" /></a>
<a href="https://huggingface.co/lxcxjxhx"><img src="https://img.shields.io/badge/HuggingFace-2c3e50?style=for-the-badge&logo=huggingface&logoColor=white" /></a>

</td></tr>
</table>
</div>

---

<div align="center">

<sub>感谢所有队友、导师和朋友在学习旅程中给予的指导与支持。</sub>

<br/>

<img src="https://komarev.com/ghpvc/?username=lxcxjxhx&style=flat-square&color=2c3e50" alt="Profile Views"/>

</div>

---

<div align="center">
<sub>本 README 通过 GitHub Actions 自动更新，英文版 README.en.md 自动翻译</sub>
</div>
